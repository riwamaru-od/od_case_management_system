...
// ------------------------------------------------------------------
// PDF出力時のページ区切り（参考情報）。
// Google スプレッドシートのPDFエクスポートAPIは、1回のリクエストで複数の
// 飛び地セル範囲を「各範囲ごとに1ページへ収めつつ複数ページに分ける」ことが
// できないため、ページ区切りはコードではなく各テンプレート側で
// 「ファイル > 印刷設定 > ページ区切りを設定」から手動で設定する運用とする
// （buildPdfExportUrl_ はテンプレート側の印刷設定にそのまま従う）。
// 各書類のページ構成は以下の通り。テンプレートの列幅・行高を変更した場合は
// ページ区切りの位置もあわせて再設定すること。
