...
// ------------------------------------------------------------------
// 書類作成時のロック（下記の範囲だけを編集可能にし、それ以外を保護する）
//
// テンプレートの体裁や自動転記されるヘッダー項目を、利用者が誤って書き換えることを防ぐ。
// 明細など、利用者が実際に入力する範囲だけを保護の対象外にする。
// スクリプト自身はファイルのオーナー（管理用アカウント）として実行されるため、
// 保護された範囲へも従来どおり転記できる。
// ------------------------------------------------------------------
// 見積書の社内作業用エリア。作成時も承認後も、常に編集できるようにする範囲。
// PDF出力範囲（A1:H54）の外側にあるため、取引先へ渡すPDFには影響しない。
//
// ただし差し戻された書類と、再作成で退避された旧版シートは、この範囲も含めて
// 全体を保護する（過去の記録として固定するため）。
...
// ロックされた範囲を編集しようとした利用者に伝えたい文言。
// シート保護の説明として設定する（Googleが表示する警告文そのものは変更できない。
// 詳細は TemplateFillService.gs の protectSheet_ のコメント参照）。
const DOC_LOCK_MESSAGE = 'この範囲の編集はロックされています。総務にロックの解除を依頼してください';
...
// 過去期のアーカイブは、その期の案件がすべて完了（最終承認または中止）してから
// この日数が経過した後に行う。完了した当日に作業ファイルから消してしまうと、
// 最終承認の取り消しなどの戻し操作ができなくなるため、猶予を置く。
// 1 なら「すべて完了した翌日」にアーカイブされる。
const ARCHIVE_WAIT_DAYS_AFTER_CLOSED = 1;
