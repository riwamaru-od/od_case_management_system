 ...
 *   2) 【利用者が各自1回】メニュー「ユーザー用システム設定 > サイドバー自動表示を有効にする」
...
function onOpen() {
  ...
  ui.createMenu('管理者用メニュー')
    ...
    .addItem('取引先名の選択肢を更新する', 'refreshClientOptionsManually')
    .addSeparator()
...
function dailyScheduledTasks() {
  ...
  runDailyTask_('取引先名の選択肢更新', refreshClientOptionsIfNeeded_);
...
/**
 * 時間主導型トリガーから毎日午前10時に呼ばれる想定のエントリーポイント。
 * 利用者が業務時間中に読むことを前提とした通知はこちらへまとめる
...
function installTriggers() {
  ...
  // 未承認案件レポート（毎月25日）・放置書類アラート（毎日）は業務時間中に読めるよう午前10時に送る
