...
| 取引先の選択 | 取引先名は「社名（担当者名）」のプルダウンから選ぶ。社名が重複していても担当者で区別できる |
...
| 請求後の再見積もり | 請求を出した後でも見積書を作り直せる。古い請求書・納品書はロックし、作り直すまで出力させない |
| PDF出力 | 出力範囲を制御し、操作履歴欄を除いた本文のみを出力。最新版のみ保持し、古いPDFは自動削除 |
...
| 差し戻された書類 | 全体を保護 |
| 再作成で退避された旧版シート | 全体を保護（あわせて社印を除去し、有効な書類と見分けられるようにする）|
| 上流が作り直されて無効になった書類 | 全体を保護（同上。作り直すまで出力できない。下記「請求後の再見積もり」参照）|
...
請求書を出した後でも、見積書の「再作成」はいつでも行える（見積書が一度でも承認されていれば、
請求済みの案件でも再作成ボタンが活性になる）。金額が変わった場合に、案件を作り直さずに
そのまま作業を続けられるようにするため。

このとき、その見積書を元に作られた**請求書・納品書は内容が古くなる**ため、自動的に
「無効」の状態にする（`invalidateDownstreamDocuments_` / ApprovalService.js）。
請求書を再作成した場合も、同じ理由で納品書が無効になる。

| 無効化で行うこと | 目的 |
| --- | --- |
| 案件シートのフラグを立てる（請求書=差し戻し日時・再承認待ち、納品書=無効化日時）| PDF出力ボタンを非活性にし、サーバー側の出力処理でも弾く |
| 出力済みのPDFを削除し、リンクを消す | 古い版のPDFが残って誤って使われるのを防ぐ |
| 書類シートを保護し、社印を外す | 無効な書類を編集・再利用できないようにし、有効な書類と見分けられるようにする |
| 請求ステータスを「未請求」へ戻す | 請求をやり直すまで最終承認させない |

無効化された書類は作り直すとフラグが消え、通常の流れ（完成 → 承認 → PDF出力）に戻る。

- 請求書: 「再作成」ボタン（見積書が再度承認されるまでは非活性）
- 納品書: 「納品書作成」ボタン。納品書には承認フローも再作成ボタンも無いため、
  無効化されている間だけこのボタンが再び活性になり、押すと見積書・請求書の再作成と同じく
  同一ファイル内へ新しいシートを積む

```
（請求済み）
  見積書 再作成 ─→ 請求書・納品書を無効化（ロック・PDF削除・請求ステータスを未請求へ）
     ↓
  見積書 完成 → 承認
     ↓
  請求書 再作成 → 完成 → 承認 → PDF出力
     ↓
  納品書 作成（作り直し）→ PDF出力 →（請求済み）→ 最終承認
```
...
| `ClientOptionsService.js` | 取引先名（社名＋担当者名）のプルダウン生成 |
...
詳細な手順は `docs/初期設定マニュアル_v2.4.docx` を参照してください。概要は以下のとおりです。
...
| `dailyScheduledTasks`（0:30） | 翌期の準備、選択肢更新（終了予定・請求予定／取引先名）、アーカイブ、バックアップ、請求予定レポート |
...
| --- | --- |
| `仕様書_v2.4.docx` | 機能仕様 |
| `初期設定マニュアル_v2.4.docx` | 導入手順 |
| `ユーザー用マニュアル_v2.4.docx` | 利用者向けの操作手順 |
| `ユーザーテスト_チェックリスト_v2.4.docx` | ユーザーテスト用のチェックリスト |
| `9月4日_修正点と動作確認チェックリスト.txt` | 9月4日の修正内容と動作確認手順 |
| `9月17日_修正点と動作確認チェックリスト.txt` | 直近の修正内容と動作確認手順（テンプレートの変更作業を含む）|
